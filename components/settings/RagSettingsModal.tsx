...
const RagSettingsModal: React.FC<RagSettingsModalProps> = ({ isOpen, onClose }) => {
  const [activeTab, setActiveTab] = useState<'rag' | 'cache'>('rag');
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setHasChanges(false);
      setActiveTab('rag'); // Reset to RAG tab when modal opens
    }
  }, [isOpen]);
      ...
      >
        ...
        <div className='flex-1 overflow-y-auto'>
          {activeTab === 'rag' ? (
            ...
            <div className='bg-gray-900 min-h-[600px] -m-6 p-6 rounded-lg'>
              <CacheManagement showHeader={false} className='p-0' />
            </div>
          )}
        </div>

        {/* Footer - Only show for RAG settings tab */}
        {activeTab === 'rag' && (
