#!/usr/bin/env node
import { saveAssistantToTurso, saveRagChunkToTurso, searchSimilarChunks } from '../services/tursoService.node.js';
...
async function testVectorSearch() {
  try {
    ...
    if (searchResults.length > 0) {
      ...
      console.log(`   - Similarity scores: ${searchResults.map(r => r.similarity.toFixed(4)).join(', ')}`);
    } else {
