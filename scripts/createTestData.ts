...
const createTestAssistants = async () => {
  try {
    ...
    const testAssistant: Assistant = {
      ...
      systemPrompt: 'You are a helpful marketing assistant specializing in content creation and strategy.',
      ragChunks: [
        {
          fileName: 'marketing_guide.txt',
          content: 'Content marketing is essential for building brand awareness. Focus on creating valuable content that resonates with your target audience.',
          vector: new Array(384).fill(0).map(() => Math.random() * 2 - 1) // 模擬向量
        },
        {
          fileName: 'seo_tips.txt',
          content: 'Search engine optimization requires keyword research, quality content, and proper meta tags. Always focus on user experience first.',
          vector: new Array(384).fill(0).map(() => Math.random() * 2 - 1) // 模擬向量
        }
      ],
    ...
    const testAssistant2: Assistant = {
      ...
      systemPrompt: 'You are a senior software engineer who helps with code reviews and best practices.',
      ragChunks: [
        {
          fileName: 'coding_standards.txt',
          content: 'Always use meaningful variable names, add comments for complex logic, and follow consistent formatting.',
          vector: new Array(384).fill(0).map(() => Math.random() * 2 - 1) // 模擬向量
        }
      ],
