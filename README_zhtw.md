...
4. **啟動開發伺服器:**
   ...
   若要在 self-signed SSL 下測試瀏覽器加密與 secure-context API，可使用：

   ```bash
   pnpm run dev:https
   ```

   應用程式會在 https://localhost:5173 運行。第一次開啟時，瀏覽器會顯示自簽憑證警告；接受一次即可繼續本機測試。
...
| `pnpm run dev:https`        | 啟動 self-signed HTTPS 開發伺服器                |
